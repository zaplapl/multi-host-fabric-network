...
export ORDERER_HOSTNAME="ip-172-31-21-115.eu-west-2.compute.internal"
export ORG1_HOSTNAME="ip-172-31-30-192.eu-west-2.compute.internal"
export ORG2_HOSTNAME="ip-172-31-17-116.eu-west-2.compute.internal"
...
function printHelp() {
  echo "Usage: "
  echo "  mhfn.sh <mode> [-c <channel name>] [-t <timeout>] [-d <delay>] [-f <docker-compose-file>] [-s <dbtype>] [-l <language>] [-i <imagetag>] [-v]"
  ...
  echo "    -v - verbose mode"
  echo "  mhfn.sh -h (print this message)"
  echo
  echo "Typically, one would first generate the required certificates and "
  echo "genesis block, then bring up the network. e.g.:"
  echo
  echo "	mhfn.sh generate -c mychannel"
  echo "	mhfn.sh up -c mychannel -s couchdb"
  echo "        mhfn.sh up -c mychannel -s couchdb -i 1.2.x"
  echo "	mhfn.sh up -l node"
  echo "	mhfn.sh down -c mychannel"
  echo "        mhfn.sh upgrade -c mychannel"
  echo
  echo "Taking all defaults:"
  echo "	mhfn.sh generate"
  echo "	mhfn.sh up"
  echo "	mhfn.sh down"
}
...
function checkPrereqs() {
  ...
  for UNSUPPORTED_VERSION in $BLACKLISTED_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      echo "ERROR! Local Fabric binary version of $LOCAL_VERSION does not match this newer version of mhfn and is unsupported. Either move to a later version of Fabric or checkout an earlier version of fabric-samples."
      exit 1
    fi

    echo "$DOCKER_IMAGE_VERSION" | grep -q $UNSUPPORTED_VERSION
    if [ $? -eq 0 ]; then
      echo "ERROR! Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match this newer version of mhfn and is unsupported. Either move to a later version of Fabric or checkout an earlier version of fabric-samples."
      exit 1
    fi
  done
}
